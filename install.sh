...
printf "  ${DIM}    |${RESET}\n"
printf "  ${DIM}    |${RESET}${DIM}--- ${RESET}${CYAN}OTLP (HTTP/JSON)${RESET}${DIM} --------------------------.${RESET}\n"
printf "  ${DIM}    |                                                  |${RESET}\n"
printf "  ${DIM}    |  ${RESET}${GREEN}SessionStart${RESET}${DIM} ---> ${RESET}session-start hook${DIM}            |${RESET}\n"
printf "  ${DIM}    |      v                  v                       v${RESET}\n"
printf "  ${DIM}    |  ${RESET}${GREEN}PreToolUse${RESET}        ${BOLD}${CYAN}warden-collector${RESET}         ${CYAN}:4319 OTLP${RESET}\n"
printf "  ${DIM}    |      v${RESET}              ${DIM}(Go binary)${RESET}          ${CYAN}receiver${RESET}\n"
printf "  ${DIM}    |  ${RESET}${DIM}[tool runs]${RESET}${DIM}            |                      |${RESET}\n"
printf "  ${DIM}    |      v${RESET}        ${CYAN}collector.sock${RESET}${DIM} (UDS)            |${RESET}\n"
printf "  ${DIM}    |  ${RESET}${GREEN}PostToolUse${RESET}${DIM}            v                      |${RESET}\n"
printf "  ${DIM}    |      v       .--- ${RESET}${BOLD}${YELLOW}collector.db${RESET}${DIM} (SQLite) --.   |${RESET}\n"
printf "  ${DIM}    |  ${RESET}${GREEN}SessionEnd${RESET}${DIM}  |  ${RESET}${YELLOW}sessions${RESET}${DIM}    ${RESET}${YELLOW}hook_events${RESET}${DIM}    |<--'${RESET}\n"
printf "  ${DIM}    |              |  ${RESET}${YELLOW}spans${RESET}${DIM}       ${RESET}${YELLOW}subagent_budgets${RESET}\n"
printf "  ${DIM}    |              |  ${RESET}${YELLOW}tokens${RESET}${DIM}      ${RESET}${YELLOW}budget-deny files${RESET}\n"
printf "  ${DIM}    |              '---------|-------------------'${RESET}\n"
printf "  ${DIM}    |                        |${RESET}\n"
printf "  ${DIM}    v                   .----+-----.${RESET}\n"
printf "  ${GREEN}statusline.sh${RESET}       ${GREEN}viewer UI${RESET}   ${GREEN}API${RESET}\n"
printf "  ${DIM}(context %%)         (:8477)   /v1/sessions${RESET}\n"
echo ""
...
if [[ -t 0 ]] && ! $DRY_RUN; then
    ...
    if [[ "${_TOUR_CHOICE:-n}" =~ ^[yY] ]]; then
        ...
        printf "  ${DIM}Key hooks: pre-tool-use (guards), post-tool-use (truncation),${RESET}\n"
        printf "  ${DIM}  read-guard (blocks binary/bundled), read-compress (extracts structure)${RESET}\n"
        printf "  ${DIM}Press Enter to continue...${RESET}" && read -r

        echo ""
        printf "${BOLD}  2/5  Go Collector${RESET}\n"
        echo "  A lightweight Go service that acts as the central backbone."
        echo "  Receives OTLP spans from Claude Code on :4319 to track tokens,"
        echo "  model, and context window utilization. Hooks POST events to it"
        echo "  via Unix socket. Manages subagent budget enforcement."
        echo ""
        printf "  ${DIM}Binary:  $COLLECTOR_BIN_PATH${RESET}\n"
        printf "  ${DIM}SQLite:  $COLLECTOR_STATE_DIR/collector.db${RESET}\n"
        printf "  ${DIM}Socket:  $COLLECTOR_STATE_DIR/collector.sock${RESET}\n"
        printf "  ${DIM}Tables:  sessions, spans, hook_events, subagent_budgets${RESET}\n"
        printf "  ${DIM}Starts on session-start, stops when idle.${RESET}\n"
        printf "  ${DIM}Press Enter to continue...${RESET}" && read -r

        echo ""
        printf "${BOLD}  3/5  Statusline${RESET}\n"
        echo "  Shows live session metrics in Claude Code's status bar."
        echo "  Queries the collector for context utilization, token counts,"
        echo "  cache stats, and active subagent count."
        echo ""
        printf "  ${DIM}Installed: $CLAUDE_DIR/statusline.sh${RESET}\n"
        printf "  ${DIM}Shows: model | context%% | in/out tokens | cache | tools | subagents${RESET}\n"
        printf "  ${DIM}Press Enter to continue...${RESET}" && read -r

        echo ""
        printf "${BOLD}  4/5  Viewer${RESET}\n"
        echo "  A web UI that reads directly from collector.db. Provides six"
        echo "  views: context gauge, request waterfall, event log, cost"
        echo "  tracking, tool breakdown, and token trend."
        echo ""
        printf "  ${DIM}Run:  python3 $WARDEN_DIR/viewer/warden-viewer.py${RESET}\n"
        printf "  ${DIM}URL:  http://localhost:8477${RESET}\n"
        printf "  ${DIM}Press Enter to continue...${RESET}" && read -r

        echo ""
        printf "${BOLD}  5/5  Configuration${RESET}\n"
        echo "  Your profile ($PROFILE) sets token limits, tool permissions,"
        echo "  and subagent budgets. These are merged into settings.json and"
        echo "  a warden.env file that hooks source at runtime."
        echo ""
